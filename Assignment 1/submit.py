...
################################
    ...
    X_temp = X.copy()
    X_temp = np.append(X_temp, np.ones((X_temp.shape[0], 1),dtype=int),axis = 1)
    X_temp = np.cumprod( np.flip( 1 - 2*X_temp , axis = 1 ), axis = 1 )
    X_temp = X_temp.transpose()
    temp = khatri_rao(X_temp, X_temp)
    X_new = khatri_rao(X_temp, temp)
...
def get_loss(y, w, x):
    return max(0, 1 - y*get_predict(w, x))

def getMBGrad( w, X, y, C, B ):
    n = y.size
    Batch_size = min( B, n )
    samples = random.sample( range(0, n), Batch_size )
    X_ = X[samples,:]
    y_ = y[samples]
    discriminant = np.multiply( (X_.dot( w.T)), y_ )
    g = np.zeros( (Batch_size,) )
    g[discriminant < 1] = -1
    delw = w + C * n/Batch_size * (X_.T * g).dot( y_ )
    return delw
...
################################
    ...
    W = np.random.randint(-100, 100, size=(729, )) #remove this code
    y_arr = get_renamed_labels(y)
    x_arr = get_features(X)
################################
# Non Editable Region Starting #
################################
    while True:
    ...
    ################################
        ...
        w = W
        DEL_W = getMBGrad(w, x_arr, y_arr, 1, 10)
        W = w-get_neta(.5,t)*DEL_W
